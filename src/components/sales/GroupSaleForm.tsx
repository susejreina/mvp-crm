...
export default function GroupSaleForm({ onSuccess }: GroupSaleFormProps) {
  ...
  const updateUser = (index: number, field: keyof SaleUser, value: string) => {
  ...
  const validateForm = (): boolean => {
    ...
    // Users validation
    const usersErrors: Array<{name?: string; email?: string}> = [];
    users.forEach((user, index) => {
      const userErrors: {name?: string; email?: string} = {};
      ...
      usersErrors[index] = userErrors;
    });
    
    if (usersErrors.some(ue => ue.name || ue.email)) {
      newErrors.users = usersErrors;
    }
  ...
  const processSubmit = async (data: FormData) => {
    ...
    try {
      ...
      onSuccess?.('Venta grupal registrada correctamente');
    ...
    <>
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-6">
          <h2 className="text-xl font-semibold text-blue-600">Comprador</h2>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-blue-600">Usuarios</h2>
            <Button
              type="button"
              variant="outline"
              onClick={addUser}
          ...
          <div className="space-y-4">
            {users.map((user, index) => (
              <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                {/* Name */}
                <div>
                  <label htmlFor={`user-name-${index}`} className="sr-only">
                    Nombre de Usuario *
                  </label>
                  <input
                    id={`user-name-${index}`}
                    ...
                    className={`w-full px-3 py-3 bg-[#E8EDF5] border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder:text-[#4A739C] text-[#4A739C] ${
                      errors.users?.[index]?.name ? 'border-red-300' : 'border-gray-200'
                    }`}
                  />
                ...
                {/* Email */}
                <div>
                  <label htmlFor={`user-email-${index}`} className="sr-only">
                    Correo *
                  </label>
                  <input
                    id={`user-email-${index}`}
                    ...
                    placeholder="Correo*"
                    className={`w-full px-3 py-3 bg-[#E8EDF5] border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder:text-[#4A739C] text-[#4A739C] ${
                      errors.users?.[index]?.email ? 'border-red-300' : 'border-gray-200'
                    }`}
                  />
                ...
                {/* Phone */}
                <div>
                  <label htmlFor={`user-phone-${index}`} className="sr-only">
                    Teléfono
                  </label>
                  <input
                    id={`user-phone-${index}`}
